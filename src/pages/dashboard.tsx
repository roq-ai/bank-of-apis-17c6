...
import { getFinancialAdvisors } from 'apiSdk/financial-advisors';
import { FinancialAdvisorInterface } from 'interfaces/financial-advisor';
import { getCreditCards } from 'apiSdk/credit-cards';
import { CreditCardInterface } from 'interfaces/credit-card';

import { FinancialAdvisorListPage as ShowCaseList } from 'pages/financial-advisors';
...
function DashboardPage() {
  ...
  const financialAdvisorStat = useSWR<PaginatedInterface<FinancialAdvisorInterface>>(
    () => `/financial-advisors`,
    () => getFinancialAdvisors({ limit: 1, offset: 0 }),
  );

  const creditCardStat = useSWR<PaginatedInterface<CreditCardInterface>>(
    () => `/credit-cards`,
    () => getCreditCards({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: financialAdvisorStat?.data?.totalCount,
      isLoading: financialAdvisorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Credit Cards',
      result: creditCardStat?.data?.totalCount,
      isLoading: creditCardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Financial Advisors
                </Text>
                <NextLink href={`financial-advisors`} passHref legacyBehavior>
                  <Button
