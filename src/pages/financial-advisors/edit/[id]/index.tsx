...
import { getFinancialAdvisorById, updateFinancialAdvisorById } from 'apiSdk/financial-advisors';
...
function FinancialAdvisorEditPage() {
  ...
  const handleSubmit = async (values: FinancialAdvisorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateFinancialAdvisorById(id, values);
      mutate(updated);
      resetForm();
